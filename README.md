# MoreWindows（TODO：优化注册流程，支持HTML注册、注册配置）
...
1. mwds没有考虑具有全局横向滚动条的页面，如果有此需求请遵循以下避坑做法（但不排除还会有其他坑）：
   ...
   - 不搭配使用`ds-a`和`ds-mov`
   - 自定义通知框的动画，不要使用默认动画
2. mwds既是声明式的组件库，又支持运行时注册。但是对于所有已经注册并释放至文档中的组件，它们在mwds中的事件和属性**已不可更改**。
   - 例子：**不可以**通过移除class将一个带有`ds-mov`class的可移动窗口变得不可移动。如果确有此需求，请删除该元素并重新向mwds声明该元素。原因如下：
     1. 实时监测组件的开销实在是太大了。
     2. 目前我所有用到mwds的项目都几乎不需要运行时的属性更改。此为低频率功能但需要大幅修改代码，所以必须等我有时间才能弄。
     3. 有很方便的脱注册方法：`mwds.unregister()`，脱注册后可重新注册。
   - 还是一句话，组件库是用来方便自己的。
...
mwds在创建实例时会自动进行一些初始化操作。之后使用`mwds`实例与mwds互动。
...
- 声明式注册有时可以让关闭JavaScript的用户得到哪怕是一点点的功能，所以推荐优先使用声明式注册。
...
`<deleteElement>`：是否删除元素。若此项为`true`，则该方法返回一个HTML DOM元素。若此项为`false`，则该方法返回一个Node。
...
```javascript
mwds.win([$("#mywin")], <position>, <isTra>, <canMove>, <canClose>, <isOnTop>, <canResize>);
```

## 参数表

添加下方的class来自定义样式。自定义样式时请注意`/*<某属性>不可更改*/`的注释，更改这些属性可能导致奇奇怪怪的问题出现。

|      参数       |    class    |            描述             |      缺省      |
| :-------------: | :---------: | :-------------------------: | :------------: |
| `[$("#mywin")]` |     无      |          窗口元素           |      必需      |
|  `<position>`   |   `ds-a`    |   窗口位置行为`absolute`    |    `fixed`     |
|    `<isTra>`    |  `ds-tra`   | 窗口背景透明，hover后变白色 | 永远为指定颜色 |
|   `<canMove>`   |  `ds-mov`   |         窗口可移动          |    不可移动    |
|  `<canClose>`   |  `ds-cls`   |   双击窗口的空闲部分关闭    |   无关闭功能   |
|   `<isOnTop>`   | `ds-ontop`  |    置于最顶层（见下方）     |  动态窗口提升  |
|  `<canResize>`  | `ds-resize` |         可调整大小          |    自动大小    |
...
|                  参数                  |    class    |          描述          |   缺省    |
| :------------------------------------: | :---------: | :--------------------: | :-------: |
|        `[$("#target-Element")]`        |     无      |   要放置提示框的元素   |           |
|            `[tooltip html]`            |     无      |  在提示框中展现的HTML  |           |
| `[direction]`（`<showtip>`不为`true`） |  `ds-tt-t`  | 在上方显示无尖端提示框 | `ds-tt-t` |
|                                        |  `ds-tt-b`  | 在下方显示无尖端提示框 | `ds-tt-t` |
|                                        |  `ds-tt-l`  | 在左方显示无尖端提示框 | `ds-tt-t` |
|                                        |  `ds-tt-r`  | 在右方显示无尖端提示框 | `ds-tt-t` |
|  `[direction]`（`<showtip>`为`true`）  | `ds-tt-t-t` | 在上方显示有尖端提示框 | `ds-tt-t` |
|                                        | `ds-tt-b-t` | 在下方显示有尖端提示框 | `ds-tt-t` |
|                                        | `ds-tt-l-t` | 在左方显示有尖端提示框 | `ds-tt-t` |
|                                        | `ds-tt-r-t` | 在右方显示有尖端提示框 | `ds-tt-t` |
...
通过`dropDown()`给特定DOM元素注册一个菜单，可以控制在右键点击特定元素时才显示菜单。
...
| 属性class  |                        描述                        |             缺省             |
| :--------: | :------------------------------------------------: | :--------------------------: |
| `ds-dd-tl` |              菜单在指针**左上角**弹出              |          `ds-dd-br`          |
| `ds-dd-tr` |              菜单在指针**右上角**弹出              |          `ds-dd-br`          |
| `ds-dd-bl` |              菜单在指针**左下角**弹出              |          `ds-dd-br`          |
| `ds-dd-br` |              菜单在指针**右下角**弹出              |         本身为缺省值         |
...
`expansion html`：在扩展框中展现的东西。

`[direction]`：扩展框的展示方向。将下文中class名称去掉`ds-exp-`作为本参数的参数值。

`[trigger]`：扩展框的触发条件。将下文中class名称去掉`ds-exp-`作为本参数的参数值。
...
<any id="target-element" class="ds-ep">
    content
    <div class="ds-exp ds-exp-t ds-exp-c">expansion html</div>
</any>
```
...
本组件库以Apache License 2.0开源。使用本组件库的任何实体必须遵循Apache License 2.0许可条款，否则其使用权将被收回，并视为侵权使用行为。
