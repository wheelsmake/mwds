...
直接使用HTML文件中的元素（拥有`.ds-dd`或`ds-dd-<br|bl|tr|tl>`的元素默认`display`均为`none`，其余请自定义）：
...
受够Material design风格的通知了——从屏幕下面出现一个黑块，上面写着一些东西；当我想选择时，刷的一下跑掉了！
……

mwds的通知框分为5种：警告通知、提醒通知、普通通知、成功通知和
