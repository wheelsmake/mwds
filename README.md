...
mwds在创建实例时会自动进行一些初始化操作。

## 特别注意

1. mwds没有考虑具有全局横向滚动条的页面（因为确实太罕见了），如果要在具有或可能具有全局横向滚动条的页面上使用，请遵循以下避坑做法（不排除还会有其他坑）：

   - 不搭配使用`ds-a`和`ds-mov`
   - 自定义通知框的动画，不要使用默认动画
2. mwds**暂时不是声明式的组件库**，而是运行时注册的组件库；并且对于所有已经注册并释放至文档中的组件，它们在mwds中的事件和属性**已不可更改**。
...
`"tooltip html"`：在提示框中展现的东西，允许~~传入已有元素（TODO）或~~HTML，**在传入HTML时不要在最外层包裹容器**。
